...
func (s *Server) handleHWSubmissionCreate(w http.ResponseWriter, r *http.Request) {
	...
	hw, err := s.HomeworkService.FindHomeworkByID(r.Context(), hwID)
	if err != nil {
		Error(w, r, err)
		return
	}

	if hw.Mode == "registered" && user == nil {
		Error(w, r, api.Errorf(api.EUNAUTHORIZED, "Log in to be able to submit"))
		return
	}

	if time.Now().Before(hw.OpenedAt) {
		Error(w, r, api.Errorf(api.EUNAUTHORIZED, "Assignment has not yet been started"))
		return
	} else if time.Now().After(hw.ClosedAt) {
