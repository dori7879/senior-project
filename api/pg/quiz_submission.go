...
func (s *QuizSubmissionService) FindQuizSubmissions(ctx context.Context, filter api.QuizSubmissionFilter) ([]*api.QuizSubmission, int, error) {
	...
	subs, n, err := findQuizSubmissions(ctx, tx, filter)
	if err != nil {
		return subs, n, err
	}

	for _, s := range subs {
		if err := attachQuizStudents(ctx, tx, s); err != nil {
			return subs, n, err
		}
	}
...
func attachQuizSubmissionAssociations(ctx context.Context, tx *Tx, sub *api.QuizSubmission) (err error) {
	...
	} else if sub.Responses, _, err = findResponses(ctx, tx, api.ResponseFilter{SubmissionID: &sub.ID}); err != nil {
		return fmt.Errorf("attach quiz submission responses: %w", err)
...
func attachQuizStudents(ctx context.Context, tx *Tx, sub *api.QuizSubmission) (err error) {
	if sub.StudentID == 0 {
		return nil
	} else if sub.Student, err = findUserByID(ctx, tx, sub.StudentID); err != nil {
